...
class SyncService implements SyncServiceInterface {
  ...
  /// Export all Isar data to JSON
  Future<Map<String, dynamic>> _exportData() async {
    if (_isar == null) {
      // Initialize Isar if not already done
      final dir = await getApplicationDocumentsDirectory();
      _isar = await Isar.open(
        [TaskSchema, NoteSchema, JournalEntrySchema],
        directory: dir.path,
      );
    }
    ...
    final journalEntries = await _isar!.journalEntrys.where().findAll();
